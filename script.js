  ...
  function initMenu() {
    ...
    const menuBottomButtons = document.querySelectorAll(".menu_bottom .circle-button_wrapper");
  ...
  function initSliders() {
    ...
    ["#articles-slider", "#tariffs-slider", "#benefits-slider"].forEach(selector => {
  ...
  function initSocialButtonAnimations() {
    gsap.utils.toArray('.social-buttons_wrapper').forEach(wrapper => {
      const circleButtons = gsap.utils.toArray(wrapper.querySelectorAll('.circle-button'))
      .filter(circleButton => circleButton.getAttribute('disable-animation') !== 'true');
    ...
    fakeButtons.forEach((fakeButton) => {
      ...
      if (trueSubmit && phoneInput && buttonText && arrowWrapper) {
        fakeButton.addEventListener('click', function () {
          ...
          const isPhoneComplete = /^\+7 \(\d{3}\) \d{3}-\d{2}-\d{2}$/.test(phoneValue);

          if (!form.checkValidity() || !isPhoneComplete) {
            phoneInput.setCustomValidity(isPhoneComplete ? "" : "Пожалуйста, введите полный номер телефона");
            form.reportValidity();
            return;
          }
...
function splitHeroHeading(element, { forResize = false } = {}) {
  try { SplitType.revert(element); } catch (_) {}
  ...
  if (logoWrapper || paragraph) {
    if (logoWrapper) {
      tl.fromTo(
        logoWrapper,
        { width: 0 },
        { width: () => logoWrapper.scrollWidth, duration: 1.2, ease: "power2.inOut" },
        0
      );
    }
...
function animateNavbarAndSocial(delay = 0) {
  ...
  if (navbar) {
    tl.fromTo(navbar, { y: "-8em" }, { y: "0em", duration: 0.8, ease: "power2.out" }, 0);
  }
  ...
  if (arrow) {
    tl.fromTo(arrow, { opacity: 0 }, { opacity: 1, duration: 0.8, ease: "power1.out" }, 0.4);
  }
  ...
  function resizeHandler() {
    ...
    timer = setTimeout(() => {
      ...
      if (Math.abs(w - lastWidth) > 8) {
        ...
        if (logoWrapper) gsap.set(logoWrapper, { width: logoWrapper.scrollWidth });
      }
    }, 150);
  }
...
function initHeroAnimations() {
  ...
  var navLink = 'yandexnavi://build_route_on_map?lat_to=' + destLat + '&lon_to=' + destLon;
  var webLink = 'https://yandex.ru/maps/?mode=routes&rtext=~' +
